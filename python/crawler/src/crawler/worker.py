...
class Result:
    ...
    def to_text(self, *, page_index: int | None = None) -> str:
        header = f"[{page_index}]: {self.url}" if page_index is not None else self.url
        lines = [header]
        if self.links:
...
class Worker:
    ...
    async def fetch(self, url: str) -> Result:
        try:
            ...
            return Result(url=url, links=[], crawlable_links=[], error=str(exc))
